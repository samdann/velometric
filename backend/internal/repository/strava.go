...
func (r *StravaRepository) CreateJob(ctx context.Context, job *model.StravaSyncJob) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO strava_sync_jobs (id, user_id, status, local_only, limit_count, offset_count, started_at, created_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`, job.ID, job.UserID, job.Status, job.LocalOnly, job.LimitCount, job.OffsetCount, job.StartedAt, job.CreatedAt)
	if err != nil {
...
func (r *StravaRepository) GetJob(ctx context.Context, id uuid.UUID) (*model.StravaSyncJob, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT id, user_id, status, local_only, limit_count, offset_count, fetched_count, updated_count, created_count,
		       error_message, started_at, fetched_at, completed_at, created_at
		FROM strava_sync_jobs WHERE id = $1
	`, id).Scan(
		&j.ID, &j.UserID, &j.Status, &j.LocalOnly, &j.LimitCount, &j.OffsetCount, &j.FetchedCount, &j.UpdatedCount, &j.CreatedCount,
